...
try:
    from SaFIN.clip import CLIP, rule_creation
    from SaFIN.common import boolean_indexing
    from SaFIN.safin import SaFIN
except ImportError:
    from clip import CLIP, rule_creation
    from common import boolean_indexing
    from safin import SaFIN
    
def main():
    boston = load_boston()
    NUM_DATA = 400
    batch_X = boston.data[:NUM_DATA]
    batch_Y = np.array([boston.target]).T[:NUM_DATA]
    ALPHA = 0.2
    BETA = 0.6
    ...
    for q in range(batch_Y.shape[1]):
