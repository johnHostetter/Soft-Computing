...
class CoreNeuroFuzzy(object):
    ...
    def input_layer_pytorch(self, x):
        """
        Singleton Fuzzifier (directly pass on the input vector to the next layer).
    ...
    def condition_layer_pytorch(self, o1):
        input_weight_shape = (self.P, max(self.J.values()))
        input_weights = np.zeros(input_weight_shape)
        # make a weight matrix that has shape of (num of input dimensions, maximum number of antecedent terms)
        for p in range(self.P):
            input_weights[p][:self.J[p]] = 1
        input_weights_tensor = torch.tensor(input_weights)
        # convert the matrix to have max # of antecedent terms' copies of each input (i.e., match input_weight_shape)
        activations_tensor = self.f1[:, None] * input_weights_tensor
        antecedent_centers_tensor = torch.tensor(self.term_dict['antecedent_centers'])
        antecedent_widths_tensor = torch.tensor(self.term_dict['antecedent_widths'])
        denominator = torch.pow(antecedent_widths_tensor, 2)
        self.f2 = torch.exp(-1.0 * (torch.pow(activations_tensor - antecedent_centers_tensor, 2) / denominator))
    ...
    def rule_base_layer_pytorch(self, o2, inference='minimum'):
        # shape is (num of rules, num of observations, num of total possible antecedents)
        rule_activations = torch.mul(torch.tensor(o2), torch.tensor(self.W_2.T[:, np.newaxis]))
        # shape is (num of observations, num of rules, num of total possible antecedents)
        rule_activations = torch.swapaxes(rule_activations, 0, 1)
        if inference == 'minimum':
            self.f3 = torch.nanmin(rule_activations, axis=2)  # the shape is (num of observations, num of rules)
        elif inference == 'product':
            self.f3 = torch.nanprod(rule_activations, axis=2)  # the shape is (num of observations, num of rules)
        return self.f3
